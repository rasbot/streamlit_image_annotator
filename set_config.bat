...
set /p categories=Set default category buttons. See the README for more info. Hit enter to use defaults "keep, delete, fix, other" or provide a comma separated list here: 
...
(for /f "tokens=1,* delims=:" %%a in (config.yml) do (
    ...
    if "%%a"=="categories" set "line= %categories%"
    echo %%a:!line!
)) > config.tmp
